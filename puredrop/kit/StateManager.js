module.exports = (device, platform) => {
	...
	const filterApi = platform.filterApi
	...
	return {
		...
		get: {
			...
			refreshState: () => {
				filterApi.getConfigurations(device.id)
					.then((configurations) => {
		...
		set: {
			...
			boilWater: () => {
				...
				return filterApi.boilWater(device.id)
			...
			configurationState: (type, state, name) => {
				...
				return filterApi.setConfiguration(device.id, type, state)
					.then(() => Promise.resolve())
					.catch(err => {
